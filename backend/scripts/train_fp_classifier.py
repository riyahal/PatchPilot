...
DB_PATH = os.path.join(BACKEND_DIR, "patchpilot.db")
...
def load_data(min_samples: int) -> pd.DataFrame:
    """Extract human-labeled findings from the SQLite database."""
    ...
    if len(df) < min_samples:
        print("⚠️ Not enough labeled data to train the model.")
        print(f"Required: {min_samples}, Found: {len(df)}")
        print(
            "Please use the UI to label more findings as True/False Positives and try again."
        )
        exit(0)
    ...
    if len(class_counts) < 2 or class_counts.min() < 2:
        print(
            "⚠️ Training requires at least TWO examples of both True Positives (0) and False Positives (1)."
        )
        print(f"Current distribution:\n{class_counts}")
        print("Please use the UI to label more findings and try again. Aborting.")
        exit(0)
...
def generate_embeddings(texts: list[str], batch_size: int = 16) -> torch.Tensor:
    """Pass text through CodeBERT to extract deep contextual embeddings."""
    device = get_device()
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="Minimum labeled examples required to run",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
        default=16,
        help="Batch size for CodeBERT (lower this if you run out of memory)",
    )
